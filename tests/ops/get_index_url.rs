...
fn default_vs_sparse() {
    for suffix in &["config", "config.toml"] {
        ...
        assert_eq!(
            get_index_url(&crates_file, "https://github.com/rust-lang/crates.io-index", false),
            Ok(("https://github.com/rust-lang/crates.io-index".into(), false, "crates-io".into()))
        );
        assert_eq!(
            get_index_url(&crates_file, "https://github.com/rust-lang/crates.io-index", true),
...
fn nonexistent() {
    for suffix in &["config", "config.toml"] {
        ...
        assert_eq!(
            get_index_url(&crates_file, "https://github.com/LoungeCPP/pir-8-emu", false),
            Err(format!(
                "Non-crates.io registry specified and no config file found at {} or {}. Due to a Cargo limitation we will not be able to \
...
fn unknown() {
    for suffix in &["config", "config.toml"] {
        ...
        assert_eq!(
            get_index_url(&crates_file, "https://github.com/LoungeCPP/pir-8-emu", false),
            Err(format!(
                "Non-crates.io registry specified and https://github.com/LoungeCPP/pir-8-emu couldn't be found in the config file at {}. \
...
fn default() {
    for suffix in &["config", "config.toml"] {
        assert_eq!(
            get_index_url(&prep_config("default", suffix), "https://github.com/rust-lang/crates.io-index", false),
...
fn from_alt_url() {
    for suffix in &["config", "config.toml"] {
        assert_eq!(
            get_index_url(&prep_config("from_alt_url", suffix), "file:///usr/local/share/cargo", false),
            Ok(("outside-the-scope-of-this-document".into(), false, "tralternative".into()))
        );
    }
}
...
fn from_name() {
    for suffix in &["config", "config.toml"] {
        assert_eq!(
            get_index_url(&prep_config("from_name", suffix), "alternative", false),
            Ok(("outside-the-scope-of-this-document".into(), false, "tralternative".into()))
        );
    }
}
...
fn sus() {
    for suffix in &["config", "config.toml"] {
        assert_eq!(
            get_index_url(&prep_config("sus", suffix), "sus", false),
...
fn dead_end() {
    for suffix in &["config", "config.toml"] {
        ...
        assert_eq!(
            get_index_url(&crates_file, "dead-end", false),
            Err(format!(
