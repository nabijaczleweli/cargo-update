...
fn intersect_packages() {
    assert_eq!(
        ops::intersect_packages(
            &[
                RegistryPackage::parse("cargo-outdated 0.2.0 (registry+https://github.com/rust-lang/crates.io-index)", vec![]).unwrap(),
...
fn crate_versions() {
    assert_eq!(
        ops::crate_versions(&fs::read("test-data/checksums-versions.json").unwrap()).unwrap(),
        vec![
            (Semver::parse("0.2.0").unwrap(), None),
            ...
            (
                ...
                Some(chrono::DateTime::parse_from_rfc3339("2016-09-21T19:20:47Z").unwrap())
            )
        ]
    );
}
